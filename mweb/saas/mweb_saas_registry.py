...
class MWebSaaSRegistry:
    ...
    @classmethod
    async def set_config(cls, tenant_key: str, config_key: str, value):
        ex_config: MWebExternalSaaSConfig | None = MWebHook.external_saas_config()
        if ex_config is not None:
            await ex_config.set_config(tenant_key=tenant_key, config_key=config_key, value=value)
        else:
            cls._in_memory_config.setdefault(tenant_key, {})[config_key] = value

    @classmethod
    async def get_config(cls, config_key: str, default=None, tenant_key: str = None):
        ex_config: MWebExternalSaaSConfig | None = MWebHook.external_saas_config()
        if ex_config is not None:
            return await ex_config.get_config(tenant_key=tenant_key, config_key=config_key, default=default)
        else:
